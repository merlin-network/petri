...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=petri \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=petrid \
...
ifneq ($(GO_SYSTEM_VERSION), $(REQUIRE_GO_VERSION))
	@echo "ERROR: Go version ${REQUIRE_GO_VERSION} is required for $(VERSION) of Petri."
	exit 1
endif
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/petrid ./cmd/petrid
endif

build-static-linux-amd64: go.sum $(BUILDDIR)/
	$(DOCKER) buildx create --name petribuilder || true
	$(DOCKER) buildx use petribuilder
	$(DOCKER) buildx build \
	...
	$(DOCKER) rm -f petribinary || true
	$(DOCKER) create -ti --name petribinary petri-amd64
	$(DOCKER) cp petribinary:/bin/petrid $(BUILDDIR)/petrid-linux-amd64
	$(DOCKER) rm -f petribinary

install: check_version go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/petrid

install-test-binary: check_version go.sum
	go install -mod=readonly $(BUILD_FLAGS_TEST_BINARY) ./cmd/petrid
...
draw-deps:
	...
	@goviz -i ./cmd/petrid -d 2 | dot -Tpng -o dependency-graph.png
...
kill-dev:
	@echo "Killing petrid and removing previous data"
	-@rm -rf ./data
	-@killall petrid 2>/dev/null
