	...
	for _, tt := range tests {
		...
		if tt.expectedErr != nil {
			...
			query, _ := keeper.Keeper.RegisteredQuery(
				suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper, sdk.WrapSDKContext(ctx),
...
func (suite *KeeperTestSuite) TestUpdateInterchainQuery() {
	...
	for i, tt := range tests {
		suite.Run(fmt.Sprintf("Case %s, %d/%d tests", tt.name, i, len(tests)), func() {
			...
			iqkeeper := suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper
...
func (suite *KeeperTestSuite) TestRemoveInterchainQuery() {
	...
	for i, tt := range tests {
		suite.Run(fmt.Sprintf("Case %s, %d/%d tests", tt.name, i, len(tests)), func() {
			...
			// Top up contract address with native coins for deposit
			bankKeeper := suite.GetPetriZoneApp(suite.ChainA).BankKeeper
			senderAddress := suite.ChainA.SenderAccounts[0].SenderAccount.GetAddress()
			suite.TopUpWallet(ctx, senderAddress, contractAddress)
	...
	tests := []struct {
		...
		},
		{
			...
			func(sender string, ctx sdk.Context) {
				...
				suite.NoError(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper.UpdateLastRemoteHeight(ctx, res.Id, ibcclienttypes.NewHeight(suite.ChainA.LastHeader.GetHeight().GetRevisionNumber(), 9999)))
		...
		},
		{
			...
			func(sender string, ctx sdk.Context) {
				...
				suite.NoError(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper.UpdateLastRemoteHeight(ctx, res.Id, ibcclienttypes.NewHeight(suite.ChainA.LastHeader.GetHeight().GetRevisionNumber()+1, 1)))
