	...
	tests := []struct {
	...
	}{
		{
			...
			func() {
				...
				_, err := keeper.Keeper.LastRemoteHeight(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper, sdk.WrapSDKContext(ctx), &iqtypes.QueryLastRemoteHeight{ConnectionId: "test"})
		...
		},
		{
			...
			func() {
				...
				oldHeight, err := keeper.Keeper.LastRemoteHeight(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper, sdk.WrapSDKContext(ctx), &iqtypes.QueryLastRemoteHeight{ConnectionId: suite.Path.EndpointA.ConnectionID})
				...
				updatedHeight, err := keeper.Keeper.LastRemoteHeight(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper, sdk.WrapSDKContext(ctx), &iqtypes.QueryLastRemoteHeight{ConnectionId: suite.Path.EndpointA.ConnectionID})
	...
	for i, tc := range tests {
		...
		suite.Run(fmt.Sprintf("Case %s, %d/%d tests", tt.name, i, len(tests)), func() {
			...
			for _, q := range tt.registeredQueries {
				q := q
				suite.NoError(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper.SaveQuery(suite.ChainA.GetContext(), &q))
			}

			resp, err := suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper.RegisteredQueries(sdk.WrapSDKContext(suite.ChainA.GetContext()), tt.req)
			suite.NoError(err)
...
func (suite *KeeperTestSuite) TestQueryResult() {
	...
	msgSrv := keeper.NewMsgServerImpl(suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper)
	...
	queryResultResponse, err := suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper.QueryResult(sdk.WrapSDKContext(ctx), &iqtypes.QueryRegisteredQueryResultRequest{
		QueryId: regQuery1.Id,
	})
	suite.NoError(err)
	...
	_, err = suite.GetPetriZoneApp(suite.ChainA).InterchainQueriesKeeper.QueryResult(sdk.WrapSDKContext(ctx), &iqtypes.QueryRegisteredQueryResultRequest{
