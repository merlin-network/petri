	...
	tests := []struct {
	...
	}{
		{
			...
			func() error {
				...
				return iqkeeper.Verifier{}.VerifyHeaders(suite.ChainA.GetContext(), suite.GetPetriZoneApp(suite.ChainA).IBCKeeper.ClientKeeper, clientID, header, nextHeader)
			},
			nil,
		},
		{
		...
		},
		{
			...
			func() error {
				...
				return iqkeeper.Verifier{}.VerifyHeaders(suite.ChainA.GetContext(), suite.GetPetriZoneApp(suite.ChainA).IBCKeeper.ClientKeeper, clientID, &oldHeader, &oldNextHeader)
			},
			nil,
		},
	}
