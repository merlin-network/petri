...
func (suite *HooksTestSuite) TestOnRecvPacketHooks() {
	...
	testCases := []struct {
		...
		{"override", func(status *testutils.Status) {
			suite.GetPetriZoneApp(suite.ChainB).HooksTransferIBCModule.
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			ack := suite.GetPetriZoneApp(suite.ChainB).HooksTransferIBCModule.
			...
			if _, ok := suite.GetPetriZoneApp(suite.ChainB).HooksTransferIBCModule.
...
func (suite *HooksTestSuite) receivePacketWithSequence(receiver, memo string, prevSequence uint64) []byte {
	...
	err := suite.GetPetriZoneApp(suite.ChainB).HooksICS4Wrapper.SendPacket(
...
func (suite *HooksTestSuite) TestFundsAreTransferredToTheContract() {
	...
	balance := suite.GetPetriZoneApp(suite.ChainA).BankKeeper.GetBalance(suite.ChainA.GetContext(), addr, localDenom)
...
func (suite *HooksTestSuite) StoreContractCode(chain *ibctesting.TestChain, addr sdk.AccAddress, path string) uint64 {
	...
	codeID, _, err := wasmkeeper.NewDefaultPermissionKeeper(suite.GetPetriZoneApp(chain).WasmKeeper).Create(chain.GetContext(), addr, wasmCode, &wasmtypes.AccessConfig{Permission: wasmtypes.AccessTypeEverybody, Address: ""})
	if err != nil {
		panic(err)
	}
...
func (suite *HooksTestSuite) InstantiateContract(chain *ibctesting.TestChain, funder sdk.AccAddress, codeID uint64, initMsg string) sdk.AccAddress {
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(suite.GetPetriZoneApp(chain).WasmKeeper)
...
func (suite *HooksTestSuite) QueryContract(chain *ibctesting.TestChain, contract sdk.AccAddress, req []byte) string {
	state, err := suite.GetPetriZoneApp(chain).WasmKeeper.QuerySmart(chain.GetContext(), contract, req)
	if err != nil {
		panic(err)
	}
	return string(state)
