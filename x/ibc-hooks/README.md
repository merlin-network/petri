...
Removed `ibc_callback` functionality since we already have similar [sudo callback mechanism](https://docs.petri.org/petri/transfer/overview#ibc-transfer-results-handover) in Transfer module.
...
* Sender: We cannot trust the sender of an IBC packet, the counterparty chain has full ability to lie about it.
  We cannot risk this sender being confused for a particular user or module address on Petri.
...
* Funds: This field is set to the amount of funds being sent over in the ICS 20 packet. One detail is that the denom in the packet is the counterparty chains representation of the denom, so we have to translate it to Petri' representation.
