...
func TestKeeper_RecordBurnedFees(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			initial:   nil,
			amount:    sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(300)},
			expected:  &sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(300)},
			wantPanic: false,
		},
		{
			desc:      "default works with existing tokens burned sums",
			initial:   &sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(100)},
			amount:    sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(300)},
			expected:  &sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(400)},
			wantPanic: false,
		},
		{
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			res := feeKeeper.GetTotalBurnedPetrisAmount(ctx)
			require.Equal(t, *tc.expected, res.Coin)
		})
	}
}

func TestKeeper_GetTotalBurnedPetrisAmount(t *testing.T) {
	for _, tc := range []struct {
		desc     string
		initial  *sdk.Coin
		expected *sdk.Coin
	}{
		{
			...
			initial:  nil,
			expected: &sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(0)},
		},
		{
			desc:     "works with existing burned fees",
			initial:  &sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(100)},
			expected: &sdk.Coin{Denom: feetypes.DefaultPetriDenom, Amount: sdk.NewInt(100)},
		},
	} {
		t.Run(tc.desc, func(t *testing.T) {
			feeKeeper, ctx := feekeeperutil.FeeburnerKeeper(t)
...
func TestKeeper_BurnAndDistribute_Ntrn(t *testing.T) {
	...
	mockBankKeeper.EXPECT().BurnCoins(ctx, consumertypes.ConsumerRedistributeName, sdk.Coins{sdk.NewCoin(feetypes.DefaultPetriDenom, sdk.NewInt(100))})
...
func setupBurnAndDistribute(t *testing.T, ctrl *gomock.Controller, coins sdk.Coins) (*keeper.Keeper, sdk.Context, *mock_types.MockBankKeeper, sdk.AccAddress) {
	redistrAddr := sdk.AccAddress("petriabcdasdf")
