...
const (
	FeeCollectorAddress = "petri1vguuxez2h5ekltfj9gjd62fs5k4rl2zy5hfrncasykzw08rezpfsd2rhm7"
...
func (suite *KeeperTestSuite) Setup() {
	...
	suite.QueryHelper = &baseapp.QueryServiceTestHelper{
		GRPCQueryRouter: suite.GetPetriZoneApp(suite.ChainA).GRPCQueryRouter(),
	...
	tokeFactoryKeeper := suite.GetPetriZoneApp(suite.ChainA).TokenFactoryKeeper
	tokeFactoryKeeper.SetParams(suite.ChainA.GetContext(), types.NewParams(
		sdktypes.NewCoins(sdktypes.NewInt64Coin(types.DefaultPetriDenom, TopUpCoinsAmount)),
		FeeCollectorAddress,
	))
...
func (suite *KeeperTestSuite) SetupTokenFactory() {
	suite.GetPetriZoneApp(suite.ChainA).TokenFactoryKeeper.CreateModuleAccount(suite.ChainA.GetContext())
}
